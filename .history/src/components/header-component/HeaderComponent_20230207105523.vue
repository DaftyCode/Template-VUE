<template>
  ...
  <div>
    <Menubar :model="items">
      ...
      <template #end>
        <InputText placeholder="Search" type="text" v-model="searchInputText" @keyup.enter="searchByWord()" />
        <div class="search-icons">
          <i v-if="!loading" class="pi pi-search" style="font-size: 1.2rem" @click="searchByWord()"></i>
          <i v-else class="pi pi-spin pi-spinner" style="font-size: 1.2rem"></i>
          <i class="pi pi-times" style="font-size: 1.2rem" @click="fetchTopNews()"></i>
        </div>
      </template>
    </Menubar>
  </div>
