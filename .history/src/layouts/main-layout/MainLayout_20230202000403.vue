<template>
  <div class="container" :class="{ 'menu-open': menuOpen }">
    <header>
      <HeaderComponent></HeaderComponent>
      <!-- <app-header (menuToggle)="menuToggle($event)" [menuOpen]="menuOpen" [menuItems]="headerMenuItems"></app-header> -->
      <!-- TODO: meter progress bar indeterminado -->
    </header>

    <!-- <app-sidenav [menuItems]="menuItems" [menuOpen]="menuOpen" class="sidenav-container"></app-sidenav> -->

    <SidenavComponent class="sidenav-container"></SidenavComponent>

    <main class="scroll-container grid-container">
      <div class="router-wrapper">
        <div v-if="menuOpen" class="menu-shadow" @click="menuToggle()"></div>
...
<script>
  ...
  export default {
    ...
    setup() {
      const { menuOpen, menuToggle } = useMainLayout();
      return {
        menuOpen,
        menuToggle,
      };
